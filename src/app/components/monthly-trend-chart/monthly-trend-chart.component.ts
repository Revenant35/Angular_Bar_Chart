...
// NOTES
//    Entries shuold be kept below 24

export class MonthlyTrendChartComponent implements OnInit {
  ...
  public timeFormat = d3.timeFormat('%b %Y');
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['data'] && changes['data'].currentValue) {
      ...
      for (let i: number = 0; i < this.raw_data.length; i++) {
        ...
        this.data.push({
          date: new Date(+this.raw_data[i].video_year, +this.raw_data[i].video_month - 1, 1),
          lower_divider: +this.raw_data[i].percent_green / total_percent,
          upper_divider: (+this.raw_data[i].percent_green + +this.raw_data[i].percent_yellow) / total_percent,
          means: [+this.raw_data[i].mean_green!, +this.raw_data[i].mean_yellow!, +this.raw_data[i].mean_red!],
          stds: [+this.raw_data[i].std_green!, +this.raw_data[i].std_yellow!, +this.raw_data[i].std_red!],
          modes: [+this.raw_data[i].mode_green!, +this.raw_data[i].mode_yellow!, +this.raw_data[i].mode_red!],
          count_videos: +this.raw_data[i].count_videos
        });
      }
      ...
      for (let datum of this.data) {
        ...
        for (let j = 0; j < this.MAIN_COLORS.length; j++) {
          ...
          path.lineTo(x, dividers[j + 1] - (j == 0 ? 0.5 : 1));
          path.closePath();
      ...
      for (let i = 0; i < this.data.length - 1; i++) {
        ...
        if (incrementMonth(this.data[i].date).getTime() == this.data[i + 1].date.getTime()) {
          ...
          for (let j = 0; j < this.SECONDARY_COLORS.length; j++) {
            ...
            path.lineTo(x, left_dividers[j + 1] - (j == 0 ? 0.5 : 1));
            path.closePath();
          ...
          for (let i = 0; i < 16; i++) {
            path.lineTo(x_center + x_rad, this.yScale(0.015625 + (.0625 * i)));
            path.lineTo(x_center, this.yScale(0.03125 + (.0625 * i)));
            path.lineTo(x_center - x_rad, this.yScale(0.046875 + (.0625 * i)));
            path.lineTo(x_center, this.yScale((.0625 * (i + 1))));
          }
          path.moveTo(x_center, this.yScale(0));
          path.closePath();
    ...
    function incrementMonth(dt: Date): Date {
      let dt_new = new Date(dt);

      dt_new.setMonth(dt_new.getMonth() + 1);

      return dt_new;
    }
  }

  public showPopup(event: MouseEvent, x: number, datum: F_DataEntry, component: MonthlyTrendChartComponent): void {
    if(this.isInitialized){
