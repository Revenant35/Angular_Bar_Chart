...
export class MonthlyTrendChartComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (hasChanged('raw_data')) {
      ...
      for (let i = 0; i < this.bar_data.length - 1; i++) {
        ...
        if (incrementMonth(this.bar_data[i].date).getTime() == this.bar_data[i+1].date.getTime()) {
          const pathData = [];
    ...
    function getDateFromRawDatum(datum: R_DataEntry): Date {
      return new Date(+datum.video_year, +datum.video_month - 1);
    }
  ...
  public showPopup(event: MouseEvent, datum: F_Bar_Data_Entry, component: MonthlyTrendChartComponent): void {
